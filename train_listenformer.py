"""Train the ListenFormer-style FLAME content predictor on LookingFace."""
...
from benchmark.listenformer import (
    ListenFormerLoss,
    LookingFaceListenFormer,
    save_listenformer_predictions,
    train_listenformer,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the ListenFormer-style FLAME content predictor")
    ...
    parser.add_argument("--checkpoint_dir", default="checkpoints/listenformer")
...
def main() -> None:
    ...
    if not train_seqs and not args.eval_only:
        raise RuntimeError(
            "No training sequences available. For ListenFormer this usually means "
    ...
    criterion = ListenFormerLoss(vel_weight=args.vel_weight)
    ...
    if not args.eval_only:
        ...
        for epoch in range(start_epoch + 1, args.epochs + 1):
            train_metrics = train_listenformer(
                model,
                train_loader,
                optimizer,
                criterion,
            ...
            if epoch % args.val_interval == 0:
                val_metrics = validate_listenformer(
                    model,
                    val_loader,
                    criterion,
                    device=device,
    ...
    prediction_cache_name = (
        f"{eval_label}_predictions_{checkpoint_tag}_{args.max_sequences if args.max_sequences > 0 else 'all'}"
    )
    prediction_cache_dir = os.path.join(args.checkpoint_dir, prediction_cache_name)
    prediction_summary = save_listenformer_predictions(
        model,
        val_loader,
        device=device,
