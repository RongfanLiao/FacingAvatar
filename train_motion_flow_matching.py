"""Train the LookingFace motion_flow_matching benchmark port."""
...
from benchmark.motion_flow_matching import (
    FlowMatchingLoss,
    MotionFlowMatchingModel,
    evaluate_saved_motion_flow_matching_metrics,
    save_motion_flow_matching_predictions,
...
def motion_flow_matching_model_config(args: argparse.Namespace) -> dict[str, object]:
    return {
        "family": "motion_flow_matching",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--time_embed_scale", type=float, default=1000.0)
    parser.add_argument("--time_sampling", choices=["uniform", "beta"], default="uniform")
    parser.add_argument("--time_beta_alpha", type=float, default=1.5)
    parser.add_argument("--time_beta_beta", type=float, default=1.5)
...
def main() -> None:
    ...
    if not train_seqs and not args.eval_only:
        raise RuntimeError(
            "No training sequences available. For motion_flow_matching this usually means "
    ...
    if args.resume_checkpoint:
        ...
        resume_metric = resume_state["primary_metric"]
        print(
    ...
    if not args.eval_only:
        ...
        for epoch in range(start_epoch + 1, args.epochs + 1):
            train_metrics = train_motion_flow_matching(
                model,
                train_loader,
                optimizer,
                criterion,
            ...
            print(
                ...
                f"train_flow={train_metrics['loss_flow']:.5f} | "
                f"train_rec={train_metrics['loss_rec']:.5f}"
            )

            if epoch % args.val_interval == 0:
                val_metrics = validate_motion_flow_matching(
                    model,
                    val_loader,
                    criterion,
                    device=device,
                ...
                print(
                    ...
                    f"val_flow={val_metrics['loss_flow']:.5f} | "
                    f"val_rec={val_metrics['loss_rec']:.5f}"
                )
                last_path = os.path.join(args.checkpoint_dir, "last.pt")
                save_checkpoint(last_path, model, optimizer, epoch, val_metrics, extra_state={"model_config": model_config})
                if val_metrics["loss_total"] < best_val:
                    best_val = val_metrics["loss_total"]
                    save_checkpoint(best_path, model, optimizer, epoch, val_metrics, extra_state={"model_config": model_config})
    ...
    prediction_summary = save_motion_flow_matching_predictions(
        model,
        val_loader,
        device=device,
    ...
    metric_results = evaluate_saved_motion_flow_matching_metrics(
