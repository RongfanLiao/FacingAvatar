"""Train the Dyadic ContinuousTransformer port on LookingFace."""
...
from benchmark.dyadic_dim import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint_dir", default="checkpoints/dyadic_dim")
...
def main() -> None:
    ...
    if not args.eval_only:
        ...
        for epoch in range(start_epoch + 1, args.epochs + 1):
            train_metrics = train_dyadic_dim(
                model,
                train_loader,
                optimizer,
                criterion,
            ...
            if epoch % args.val_interval == 0:
                val_metrics = validate_dyadic_dim(
                    model,
                    val_loader,
                    criterion,
                    device=device,
    ...
    metric_results = evaluate_dyadic_dim_metrics(
        model,
        val_loader,
        device=device,
        use_amp=use_amp,
