"""Pre-extract wav2vec2 audio features from all LookingFace MP4 files.

Steps:
  1. Scan data/LookingFace/ for *_left.mp4 across all categories
  ...
  4. Rebuild manifest → data/manifest.json
...
from config import AUDIO_DIR, AUDIO_SR, LOOKINGFACE_DIR, VIDEO_FPS, WAV2VEC_EMB_DIR
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pre-extract wav2vec2 features from LookingFace data")
    ...
    # Scan for left MP4s across all LookingFace categories
    left_mp4s = scan_left_mp4s(LOOKINGFACE_DIR)
    print(f"Found {len(left_mp4s)} left MP4 files in {LOOKINGFACE_DIR}")
